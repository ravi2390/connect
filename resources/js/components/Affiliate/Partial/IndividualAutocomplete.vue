...
export default {
    ...
    methods: {
        ...
        getSearchData(val) {
            this.isLoading = true;
            const url = '/api/v2/aggregate/individual/autocomplete?scope=global&search='+val+'&affiliateId='+this.affiliateId+'&filter='+this.filter+'&affiliateParentFilter='+this.affiliateParentFilter;
            ...
            axios.get('/api/v2/aggregate/individual/autocomplete?scope=global&id=' + this.modelValue + '&affiliateId=' + this.affiliateId)
                ...
                .then(() => {
                    if (this.items.length > 0) {
                        this.selected = this.items[0].IndividualId;
                    }
                })
                .catch(err => {
                    console.log(err);
