<template>
    ...
    >
        ...
        <template v-slot:[`item.CanContact`]="{ item }">
            <span>
                <v-checkbox :model-value="item.CanContact" disabled></v-checkbox>
            </span>
        </template>
        <template v-slot:[`item.AffiliateEmailId`]="{ item }">
            <span>
                <v-btn @click="$emit('edit-email', item)">Edit</v-btn>
            </span>
        </template>
    </v-data-table>
</template>
